...
class RegisterController extends Controller
{
    ...
    public function store(RegisterRequest $request, ActivityLogger $logger): RedirectResponse
    {
        ...
        $user=DB::transaction(function() use ($data) {
            ...
            if ($user->isSeller()) {
                Umkm::create(['user_id'=>$user->id,'nama_umkm'=>$data['nama_umkm'],'pemilik'=>$user->nama_lengkap,
                    'alamat'=>$data['alamat']??'Desa Moncongloe Lappara','no_hp'=>$user->no_hp,'status'=>'aktif']);
            }
            return $user;
        });
