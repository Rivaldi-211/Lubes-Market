...
class ProductController extends Controller
{
    ...
    public function update(ProductRequest $request, Produk $produk, ActivityLogger $logger){ $data=$request->safe()->except('foto'); if($request->hasFile('foto')){ if($produk->foto) Storage::disk('public')->delete($produk->foto); $data['foto']=$request->file('foto')->store('products','public'); } if((int)$data['stok_jumlah']===0)$data['stok_status']='Habis'; $produk->update($data); $logger->log("Admin memperbarui produk {$produk->nama_produk}",$request->user(),$request->ip()); return redirect()->route('admin.products.index')->with('success','Produk diperbarui.'); }
    public function destroy(Request $request, Produk $produk, ActivityLogger $logger){ if($produk->pesanan()->exists()) throw ValidationException::withMessages(['produk'=>'Produk dengan riwayat transaksi tidak dapat dihapus. Ubah stok menjadi Habis.']); if($produk->foto)Storage::disk('public')->delete($produk->foto); $name=$produk->nama_produk; $produk->delete(); $logger->log("Admin menghapus produk {$name}",$request->user(),$request->ip()); return back()->with('success','Produk dihapus.'); }
}
