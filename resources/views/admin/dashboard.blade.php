...
@section('content')
    ...
    <div
        style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px; align-items: start; margin-bottom: 24px;">
        @php
        ...
        <section class="data-panel">
            ...
            @if($topProducts->isEmpty())
                <x-empty-state title="Belum ada data statistik"
                    text="Statistik menu terlaris desa akan tampil setelah ada transaksi diproses." />
            @else
                <div class="stats-chart-wrapper">
                        ...
                        <div class="stats-chart-left-col">
                            <div class="stats-chart-canvas-box">
                                ...
                                <div class="stats-chart-center-text" id="adminChartCenterText">
                                    ...
                                    <strong id="adminCenterValue">{{ number_format($totalTerjualSum, 0, ',', '.') }} <span
                                            style="font-size:11px; font-weight:600;">porsi</span></strong>
                                </div>
                            </div>
                            ...
                            @foreach($topProducts as $rank => $prod)
                                ...
                                <div class="stats-legend-item" data-index="{{ $rank }}" data-name="{{ $prod->nama_produk }}">
                                    ...
                                    <div class="stats-legend-right">
                                        <span class="stats-legend-qty"
                                            data-terjual="{{ number_format($terjual, 0, ',', '.') }} porsi ({{ $pctTerjual }}%)"
                                            data-omzet="{{ $pctOmzet }}%">
                    ...
                    @if($totalTerjualSum == 0)
                        <div
                            style="font-size: 11.5px; color: #7c847e; background: #fbfaf7; border: 1px dashed #d9d5cb; padding: 8px 14px; border-radius: 8px; display: flex; align-items: center; gap: 8px;">
                            <i class="bi bi-info-circle" style="color: #173d2b; font-size: 14px;"></i>
                            <span>Diagram pie siap aktif otomatis saat pesanan dengan status <strong>Diproses</strong> atau
                                <strong>Selesai</strong> bertambah.</span>
                        </div>
                    @endif
                </div>
            @endif
