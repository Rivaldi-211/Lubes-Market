...
<section class="inner-hero compact-hero">
    <div class="shell">
        ...
        <div class="inner-hero-row">
            ...
            <p>Periksa jumlah produk sebelum melanjutkan ke checkout.</p>
        </div>
    </div>
</section>
    ...
    <div class="shell">
        ...
        @else
            <div class="cart-layout">
                <form class="cart-list" method="post" action="{{ route('cart.update') }}">
                    @csrf
                    @method('PATCH')
                    ...
                    @foreach($items as $item)
                        ...
                        <article class="cart-row">
                            ...
                            <label class="cart-qty">
                                Jumlah
                                <input type="number" name="jumlah_cart[{{ $product->id }}]" min="0" max="{{ $product->stok_jumlah }}" value="{{ $item['quantity'] }}">
                            </label>
                    ...
                    <div class="cart-actions">
                        <a class="outline-link" href="{{ route('catalogue') }}"><i class="bi bi-arrow-left"></i> Kembali ke Katalog</a>
                        <button class="outline-link" type="submit"><i class="bi bi-arrow-repeat"></i> Perbarui jumlah</button>
                    </div>
                </form>
                ...
                <aside class="cart-summary">
                    ...
                    <p>Harga belum termasuk biaya pengantaran bila ada. Metode pembayaran dipilih pada langkah berikutnya.</p>
                    
                    @auth
                        @if(auth()->user()->isBuyer())
                            <a class="button button-dark wide" href="{{ route('checkout.create') }}">Lanjut checkout <i class="bi bi-arrow-right"></i></a>
                        @else
                    ...
                    <form method="post" action="{{ route('cart.clear') }}">
                        @csrf
                        @method('DELETE')
                        <button class="clear-cart" type="submit">Kosongkan keranjang</button>
                    </form>
                </aside>
            </div>
