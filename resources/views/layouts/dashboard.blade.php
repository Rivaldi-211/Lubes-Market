...
@if(auth()->user()->isAdmin())
    <div class="sidebar-section" data-section-id="admin-utama">
        ...
        <div class="sidebar-section-body">
            ...
            <a class="{{ request()->routeIs('admin.profile.*')?'active':'' }}" href="{{ route('admin.profile.edit') }}"><i class="bi bi-person-gear"></i>Profil &amp; Akun</a>
...
<div class="sidebar-backdrop" data-sidebar-backdrop></div><main class="dashboard-main"><header class="dashboard-topbar"><button class="sidebar-trigger" data-sidebar-open aria-label="Buka menu"><i class="bi bi-list"></i></button><div><small>@yield('eyebrow','LUDES-MARKET')</small><strong>@yield('page_title','Dashboard')</strong></div><a class="topbar-user" href="{{ auth()->user()->isSeller() ? route('seller.profile.edit') : (auth()->user()->isBuyer() ? route('buyer.profile.edit') : route('admin.profile.edit')) }}" style="text-decoration:none;color:inherit;" title="Buka pengaturan profil">@if(auth()->user()->foto_profil)<img src="{{ asset('storage/' . auth()->user()->foto_profil) }}" alt="{{ auth()->user()->nama_lengkap }}" style="width:34px;height:34px;border-radius:50%;object-fit:cover;flex-shrink:0;">@else<span>{{ strtoupper(substr(auth()->user()->nama_lengkap,0,1)) }}</span>@endif<div><b>{{ auth()->user()->nama_lengkap }}</b><small>{{ ucfirst(auth()->user()->role) }}</small></div></a></header><div class="dashboard-content"><x-flash/>@yield('content')</div></main></div><script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script><script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js"></script><script src="{{ asset('assets/js/app.js') }}"></script><script src="{{ asset('assets/js/dashboard.js') }}"></script>@if(auth()->check() && auth()->user()->isSeller())<script src="{{ asset('assets/js/seller-notifications.js') }}"></script>@endif @stack('scripts')</body></html>
