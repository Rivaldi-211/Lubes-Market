    ...
    @if($transactions->count())
        <div class="order-stack">
            @php
                ...
                $rawAdminPhone = (isset($adminContact) && is_object($adminContact)) ? ($adminContact->phone ?: '081234500001') : '081234500001';
                $adminPhoneFormatted = (isset($adminContact) && is_object($adminContact)) ? ($adminContact->wa_phone ?: '6281234500001') : '6281234500001';
            @endphp

            @foreach($transactions as $item)
                @if($item['type'] === 'keroyokan')
                    {{-- 1. KONSOLIDASI TRANSAKSI PAKET KEROYOKAN (1 CARD SESUAI TEMA) --}}
                    @php
                        $batchCode = $item['code'];
                        $adminWaMsg = "Halo Admin LUDES-MARKET, saya {$buyerName} ingin meminta bantuan terkait Paket Keroyokan {$batchCode} ({$item['kelompok_nama']}). Mohon bantuannya untuk pengecekan status pengiriman. Terima kasih!";
                    ...
                    <article class="buyer-order">
                        <div class="buyer-order-top">
                            <div>
                                <small style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 4px;">
                                    <span>Keroyokan {{ $batchCode }} · {{ optional($item['tanggal_pesan'])->format('d M Y, H:i') }}</span>
                                    <span style="background: #dcfce7; color: #15803d; font-weight: 800; font-size: 10px; padding: 2px 8px; border-radius: 999px; text-transform: uppercase; letter-spacing: 0.04em; display: inline-flex; align-items: center; gap: 4px;">
                                        <i class="bi bi-people-fill"></i> PAKET KEROYOKAN RESMI
                                    </span>
                                    @if($item['kategori_nama'])
                                        <span style="background: #f1f5f9; color: #475569; font-weight: 700; font-size: 10px; padding: 2px 7px; border-radius: 6px;">
                                            {{ $item['kategori_nama'] }}
                                        </span>
                                    @endif
                                </small>
                                <h3 style="margin: 4px 0 2px;">{{ $item['kelompok_nama'] }}</h3>
                                <p>{{ $item['box_count'] }} Box · Total {{ $item['orders']->sum('jumlah') }} item gabungan dari {{ $item['orders']->pluck('produk.umkm')->unique('id')->count() }} Mitra UMKM</p>
                            </div>
                            <x-status-badge :status="$item['status']"/>
                        </div>

                        {{-- Keroyokan Packaging Notice --}}
                        <div style="background: #f4fbf6; border: 1px solid #cbf0d8; color: #185832; border-radius: 10px; padding: 10px 14px; margin: 12px 0 14px; font-size: 12px; display: flex; align-items: flex-start; gap: 10px; line-height: 1.5;">
                            <i class="bi bi-box-seam-fill" style="font-size: 16px; color: #16a34a; margin-top: 1px; flex-shrink: 0;"></i>
                            <span><strong>Paket Keroyokan:</strong> Seluruh produk mitra di bawah telah disatukan dan dikemas rapi ke dalam <strong>{{ $item['box_count'] }} Box berlabel resmi LUDES-MARKET</strong> di Platform Hub (Pengiriman satu pintu).</span>
                        </div>

                        {{-- Included Products List --}}
                        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 14px; margin: 12px 0 16px; display: flex; flex-direction: column; gap: 8px;">
                            <span style="font-size: 11px; font-weight: 800; color: #475569; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;"><i class="bi bi-box-seam" style="color: #16a34a;"></i> Rincian Komposisi Isi Paket Box:</span>
                            @foreach($item['orders'] as $subOrder)
                                <div style="display: flex; justify-content: space-between; align-items: center; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 10px 14px; gap: 12px; flex-wrap: wrap;">
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        @if($subOrder->produk?->foto)
                                            <img src="{{ asset('storage/'.$subOrder->produk->foto) }}" alt="{{ $subOrder->produk->nama_produk }}" style="width: 36px; height: 36px; border-radius: 8px; object-fit: cover; border: 1px solid #e5e7eb;">
                                        @else
                                            <div style="width: 36px; height: 36px; border-radius: 8px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 700; color: #64748b; border: 1px solid #e5e7eb;">
                                                {{ strtoupper(substr($subOrder->produk?->nama_produk ?? 'P', 0, 1)) }}
                                            </div>
                                        @endif
                                        <div>
                                            <strong style="font-size: 13.5px; color: #0f172a; display: block;">{{ $subOrder->produk?->nama_produk }}</strong>
                                            <small style="color: #64748b; font-size: 11.5px;">{{ $subOrder->produk?->umkm?->nama_umkm }} · {{ $subOrder->jumlah }} unit dialokasikan (Rp{{ number_format((float)$subOrder->produk?->harga, 0, ',', '.') }}/pcs)</small>
                                        </div>
                                    </div>
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        <strong style="font-size: 13px; color: #0f172a;">Rp{{ number_format((float)($subOrder->produk?->harga * $subOrder->jumlah), 0, ',', '.') }}</strong>
                                        @if($subOrder->status === 'Selesai')
                                            @if($subOrder->ulasan)
                                                <span class="review-done" style="font-size: 11px;"><i class="bi bi-star-fill"></i> {{ $subOrder->ulasan->rating }}/5</span>
                                            @else
                                                <button class="btn-primary" type="button" data-review-open="review-{{ $subOrder->id }}" style="padding: 5px 10px; font-size: 11px;"><i class="bi bi-star"></i> Ulas</button>
                                            @endif
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="buyer-order-meta">
                            <span>Metode <b>{{ $item['metode_pembayaran'] }}</b></span>
                            <span>Total <b>Rp{{ number_format((float)$item['total_harga'],0,',','.') }}</b></span>
                            <span>Tujuan <b>{{ $item['alamat_pengiriman'] ?: '-' }}</b></span>
                        </div>

                        {{-- Bank Snapshot Transfer Info --}}
                        @if($item['metode_pembayaran'] === 'Transfer' && $item['rekening_bank_snapshot'])
                            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px 14px; margin: 10px 0; font-size: 12px; display: flex; align-items: center; justify-content: space-between;">
                                <div>
                                    ...
                                    <strong style="color: #0f172a; font-size: 13px;">{{ $item['rekening_bank_snapshot'] }}</strong>
                                </div>
                                <button type="button" class="btn-secondary" style="padding: 4px 8px; font-size: 11px;" onclick="navigator.clipboard.writeText('{{ $item['rekening_bank_snapshot'] }}'); this.innerText='Tersalin!'; setTimeout(()=>this.innerText='Salin', 2000)"><i class="bi bi-copy"></i> Salin</button>
                            </div>
                        @endif

                        {{-- FEATURE: Delivery Validation Prompt for 'Diproses' Keroyokan --}}
                        @if($item['status'] === 'Diproses')
                            <div class="delivery-validation-card">
                                <div class="dvc-main">
                                    <div class="dvc-badge">
                                        <span class="pulse-dot"></span>
                                        <i class="bi bi-truck"></i> Paket Keroyokan Sedang Diproses / Dikirim
                                    </div>
                                    <h4 class="dvc-question">Apakah barang pesanan Paket Keroyokan Anda sudah sampai?</h4>
                                    <p class="dvc-desc">Pastikan seluruh box pesanan telah Anda terima dalam kondisi baik. Jika belum sampai atau ada kendala, hubungi Admin Platform.</p>
                                </div>
                                <div class="dvc-buttons">
                                    <form method="post" action="{{ route('buyer.orders.confirm-received-batch', $item['batch_id']) }}" onsubmit="return confirm('Apakah Anda yakin barang untuk Paket Keroyokan {{ $batchCode }} sudah sampai dan Anda terima dengan baik?')">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    <button type="button" class="btn-confirm-no" data-contact-open="contact-batch-{{ $item['batch_id'] }}">
                        ...
                        <div class="row-actions">
                            <a class="btn-secondary" href="{{ route('receipt.show', $item['first_order']) }}" target="_blank"><i class="bi bi-printer"></i> Nota</a>

                            @if($item['status'] === 'Menunggu')
                                <form method="post" action="{{ route('buyer.orders.cancel-batch', $item['batch_id']) }}" onsubmit="return confirm('Batalkan Paket Keroyokan ini?')">
                                    @csrf
                                    @method('PATCH')
                            ...
                            @if(in_array($item['metode_pembayaran'],['Transfer','QRIS']) && $item['status'] !== 'Dibatalkan')
                                <form class="proof-form" method="post" enctype="multipart/form-data" action="{{ route('buyer.orders.proof-batch', $item['batch_id']) }}">
                                    @csrf
                                    <label class="btn-secondary">
                                        <i class="bi bi-upload"></i> {{ $item['bukti_pembayaran'] ? 'Ganti bukti' : 'Upload bukti' }}
                            ...
                            {{-- Chat / Bantuan Button --}}
                            <button type="button" class="btn-secondary" data-contact-open="contact-batch-{{ $item['batch_id'] }}" title="Hubungi Admin Platform">
                                <i class="bi bi-chat-left-text-fill"></i> Chat / Bantuan
                            </button>
                        </div>

                        {{-- Modal Bantuan Batch Keroyokan --}}
                        <dialog id="contact-batch-{{ $item['batch_id'] }}" class="contact-support-dialog" style="border-radius: 18px; border: 1px solid #e2e8f0; max-width: 520px; width: 92%; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); padding: 0; overflow: hidden;">
                                ...
                                <div style="display: flex; align-items: flex-start; justify-content: space-between;">
                                    <div>
                                        <span style="display: inline-flex; align-items: center; gap: 6px; font-size: 10px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; background: rgba(255,255,255,0.18); padding: 3px 10px; border-radius: 999px; color: #fde047;">
                                            <i class="bi bi-shield-check"></i> Layanan Bantuan Paket Keroyokan
                                        </span>
                                        <h3 style="margin: 8px 0 2px; font-size: 1.25rem; font-weight: 800; color: #ffffff;">Butuh Bantuan Paket Keroyokan?</h3>
                                        <p style="margin: 0; font-size: 12px; color: #d1fae5;">{{ $batchCode }} · {{ $item['kelompok_nama'] }}</p>
                                    </div>
                                    <button type="button" data-contact-close="contact-batch-{{ $item['batch_id'] }}" style="font-size: 26px; color: #a7f3d0; line-height: 1; border: none; background: none; cursor: pointer; padding: 0 4px;">&times;</button>
                                </div>
                            </div>
                            ...
                            <div class="cs-dialog-body" style="padding: 20px 24px; background: #fafafa;">
                                <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 12px 16px; margin-bottom: 16px; font-size: 12px; line-height: 1.6;">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                        <span style="color: #6b7280;">Pengelola / Pengemas:</span>
                                        <strong style="color: #111827;">Platform Hub LUDES-MARKET</strong>
                                    </div>
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                        <span style="color: #6b7280;">Status:</span>
                                        <span style="font-weight: 700; color: #1e40af;">{{ $item['status'] }}</span>
                                    </div>
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                        <span style="color: #6b7280;">Total Tagihan:</span>
                                        <strong style="color: #111827;">Rp{{ number_format($item['total_harga'], 0, ',', '.') }} ({{ $item['first_order']->isPaid() ? 'Lunas' : 'Belum Lunas' }})</strong>
                                    </div>
                                    <div style="display: flex; justify-content: space-between;">
                                        <span style="color: #6b7280;">Tujuan:</span>
                                        <span style="color: #111827; text-align: right; max-width: 60%; font-weight: 500;">{{ $item['alamat_pengiriman'] ?: '-' }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="cs-dialog-footer" style="background: #f1f5f9; padding: 12px 24px; display: flex; justify-content: flex-end; gap: 10px; border-top: 1px solid #e2e8f0;">
                                <button type="button" class="btn-secondary" data-contact-close="contact-batch-{{ $item['batch_id'] }}" style="font-size: 12px; padding: 6px 16px;">Tutup</button>
                            </div>
                        </dialog>

                        {{-- Review Dialogs for each sub-order in batch --}}
                        @foreach($item['orders'] as $subOrder)
                            @if($subOrder->status === 'Selesai' && !$subOrder->ulasan)
                                <dialog id="review-{{ $subOrder->id }}" class="review-dialog" style="border-radius: 16px; border: 1px solid #e2e8f0; max-width: 480px; width: 90%;">
                                    <form method="post" action="{{ route('buyer.orders.review',$subOrder) }}" style="padding: 24px;">
                                        @csrf
                                        <div class="dialog-head" style="margin-bottom: 20px;">
                                            <div>
                                                ...
                                                <h3 style="margin: 4px 0 0; font-size: 1.25rem; font-weight: 800; color: #0f172a;">{{ $subOrder->produk?->nama_produk }}</h3>
                                            </div>
                                            <button type="button" data-review-close="review-{{ $subOrder->id }}" style="font-size: 24px; color: #94a3b8; line-height: 1; border: none; background: none; cursor: pointer;">&times;</button>
                                        </div>
                ...
                @else
                    {{-- 2. TRANSAKSI PRODUK REGULER BIASA --}}
                    @php
                        $order = $item['order'];
                        $orderCode = $item['code'];
                    ...
                    <article class="buyer-order">
                        ...
                        {{-- Delivery Validation Prompt for Regular 'Diproses' --}}
