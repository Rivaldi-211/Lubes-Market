...
class NavigationAndUploadSecurityTest extends TestCase
{
    ...
    public function test_role_navigation_shows_only_relevant_operational_links(): void
    {
        ...
        $this->actingAs($seller)->get(route('seller.dashboard'))->assertSee('Profil')->assertSee('Laporan')->assertDontSee('Pengguna');
    ...
    {
        Storage::fake('public'); $this->seed(BumdesDemoSeeder::class); $seller=User::where('role','penjual')->firstOrFail();
        $this->actingAs($seller)->post(route('seller.products.store'),['kategori_id'=>1,'nama_produk'=>'Foto Aman','harga'=>10000,'stok_status'=>'Ready','stok_jumlah'=>5,'foto'=>UploadedFile::fake()->create('foto.jpg',50,'image/jpeg')])->assertRedirect();
